-- ===========================================================================
-- Sistema de Control de Tickets — instalación completa en el esquema `public`.
--
-- Crea la estructura entera desde cero: tipos, tablas, índices, funciones,
-- vistas y el ciclo automático de sorteos. Al final deja creado el usuario
-- administrador para poder entrar.
--
-- ES PARA UNA BASE VACÍA. Si ya hay una instalación, primero
-- `supabase/borrar-todo.sql`.
--
-- POR QUÉ `public` Y NO `allan`
-- ----------------------------
-- El sistema nació sobre un esquema propio, `allan`. Supabase sólo expone por
-- la API los esquemas que se declaran en Settings → API, y `public` viene
-- expuesto de fábrica; con un esquema propio hay que acordarse de añadirlo, y
-- olvidarlo produce un sistema que compila, arranca y no encuentra ni una
-- tabla. Esta versión evita ese paso.
--
-- Generado desde las 52 migraciones de `supabase/migrations/`, que siguen
-- siendo la fuente de verdad de POR QUÉ cada cosa es como es: los comentarios
-- de cada bloque explican las decisiones y los errores que las motivaron.
-- ===========================================================================

-- >>>>>>>>>>>>>>>>>>>>  migrations/0001_esquema_public.sql  <<<<<<<<<<<<<<<<<<<<
...
create type public.estado_sorteo as enum (
  'programado',
  'abierto',
  'cerrado',
  'liquidado'
...
create type public.hora_sorteo as enum ('11:00', '15:00', '20:00');

create type public.canal_ticket as enum ('movil', 'ocr');
...
comment on column public.vendedor.activo is
...
create table public.parametro_vendedor (
  id              uuid primary key default gen_random_uuid(),
...
create unique index parametro_vendedor_vigente
  on public.parametro_vendedor (vendedor_id)
  where vigente_hasta is null;

create index parametro_vendedor_historico
  on public.parametro_vendedor (vendedor_id, vigente_desde desc);
...
create table public.sorteo (
  id             uuid primary key default gen_random_uuid(),
  fecha          date not null,
  hora           public.hora_sorteo not null,
  estado         public.estado_sorteo not null default 'programado',
...
create index sorteo_fecha on public.sorteo (fecha desc, hora);
create index sorteo_estado on public.sorteo (estado) where estado in ('abierto', 'cerrado');
...
create table public.cupo_numero (
  sorteo_id   uuid not null references public.sorteo(id) on delete cascade,
...
comment on constraint cupo_no_excedido on public.cupo_numero is
...
create table public.dispositivo (
  ...
  vendedor_id    uuid references public.vendedor(id),
...
create table public.cuota_dispositivo (
  sorteo_id      uuid not null references public.sorteo(id) on delete cascade,
...
create table public.ticket (
  ...
  vendedor_id    uuid not null references public.vendedor(id),
  canal          public.canal_ticket not null,
...
create index ticket_sorteo_vendedor on public.ticket (sorteo_id, vendedor_id);
create index ticket_vendedor_fecha  on public.ticket (vendedor_id, creado_en desc);
create index ticket_vigentes        on public.ticket (sorteo_id) where anulado_en is null;
...
create index linea_ticket on public.linea (ticket_id);
create index linea_numero on public.linea (numero);
create index linea_ganadoras on public.linea (ticket_id) where gana;
...
create table public.lote_ocr (
  ...
  estado           public.estado_lote not null default 'cargado',
  validado_por     uuid,
...
comment on column public.lote_ocr.total_declarado is
...
alter table public.ticket
  ...
  foreign key (lote_ocr_id) references public.lote_ocr(id);
...
set search_path = public
as $$
  select rol from public.usuario_perfil where id = auth.uid();
$$;

create or replace function public.fn_vendedor_actual()
returns uuid
language sql
stable
security definer
set search_path = public
as $$
  select vendedor_id from public.usuario_perfil where id = auth.uid();
$$;
...
create or replace function public.fn_abrir_sorteo(
  p_sorteo_id          uuid,
...
begin
  ...
  update public.sorteo set estado = 'abierto' where id = p_sorteo_id;

  perform public.fn_auditar('sorteo', p_sorteo_id, 'abrir', 'estado',
...
create or replace function public.fn_cerrar_sorteo(p_sorteo_id uuid)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  update public.sorteo set estado = 'cerrado' where id = p_sorteo_id;

  perform public.fn_auditar('sorteo', p_sorteo_id, 'cerrar', 'estado', 'abierto', 'cerrado');
end;
$$;
...
begin
  ...
  returning public.ticket.id into v_ticket_id;
...
create or replace function public.fn_reservar_cuota(
  p_sorteo_id      uuid,
...
declare
  v_filas integer;
begin
  insert into public.cuota_dispositivo (sorteo_id, dispositivo_id, numero, asignado)
  ...
  perform public.fn_auditar('cuota_dispositivo', p_dispositivo_id, 'reservar',
...
create or replace function public.fn_cupo_disponible(
  p_sorteo_id   uuid,
  p_vendedor_id uuid,
  p_numero      smallint
) returns numeric
...
alter table public.parametro_vendedor  enable row level security;
alter table public.sorteo              enable row level security;
alter table public.cupo_numero         enable row level security;
alter table public.dispositivo         enable row level security;
alter table public.cuota_dispositivo   enable row level security;
alter table public.ticket              enable row level security;
alter table public.linea               enable row level security;
alter table public.liquidacion         enable row level security;
alter table public.lote_ocr            enable row level security;
...
create policy perfil_propio on public.usuario_perfil
  for select to authenticated
  using (id = auth.uid());

create policy perfil_admin on public.usuario_perfil
  for select to authenticated
...
create policy vendedor_lectura on public.vendedor
  for select to authenticated using (true);

create policy sorteo_lectura on public.sorteo
  for select to authenticated using (true);

create policy cupo_lectura on public.cupo_numero
  for select to authenticated using (true);
...
create policy parametro_propio on public.parametro_vendedor
  for select to authenticated
  using (
    vendedor_id = public.fn_vendedor_actual()
    or public.fn_rol_actual() in ('administrador', 'auditor')
  );
...
create policy ticket_propio on public.ticket
  for select to authenticated
  using (
    vendedor_id = public.fn_vendedor_actual()
    or public.fn_rol_actual() in ('administrador', 'auditor', 'digitador')
  );

create policy linea_por_ticket on public.linea
  for select to authenticated
  using (
    exists (
      ...
      where t.id = linea.ticket_id
        and (
          t.vendedor_id = public.fn_vendedor_actual()
          or public.fn_rol_actual() in ('administrador', 'auditor', 'digitador')
        )
    )
  );

create policy liquidacion_propia on public.liquidacion
  for select to authenticated
  using (
    vendedor_id = public.fn_vendedor_actual()
    or public.fn_rol_actual() in ('administrador', 'auditor')
...
create policy dispositivo_propio on public.dispositivo
  for select to authenticated
  using (
    vendedor_id = public.fn_vendedor_actual()
    or public.fn_rol_actual() in ('administrador', 'auditor')
...
create policy cuota_propia on public.cuota_dispositivo
  for select to authenticated
  using (
    exists (
      ...
      where d.id = cuota_dispositivo.dispositivo_id
        and (
          d.vendedor_id = public.fn_vendedor_actual()
          or public.fn_rol_actual() in ('administrador', 'auditor')
        )
    )
  );
...
create policy lote_digitador on public.lote_ocr
  for select to authenticated
  using (public.fn_rol_actual() in ('digitador', 'administrador', 'auditor'));

-- --- Auditoría -------------------------------------------------------------
-- Sólo lectura, y sólo para quien audita. Nadie tiene INSERT directo: se
-- escribe exclusivamente desde public.fn_auditar.
...
create policy escenario_lectura on public.escenario
  for select to authenticated
  using (public.fn_rol_actual() in ('administrador', 'auditor'));
...
-- política sobre `public.sorteo`. Es la única puerta abierta al público, y el
...
create view public.v_resultado_publico as
  select fecha, hora, numero_ganador
  from public.sorteo
  where estado = 'liquidado'
    and numero_ganador is not null;

grant select on public.v_resultado_publico to anon, authenticated;

comment on view public.v_resultado_publico is
...
drop function if exists public.fn_registrar_ticket(
  uuid, uuid, jsonb, double precision, double precision, uuid, public.canal_ticket, uuid
);

create function public.fn_registrar_ticket(
  p_sorteo_id      uuid,
  p_vendedor_id    uuid,
  p_lineas         jsonb,
  p_lat            double precision default null,
...
-- `premios` coincidía con la columna de public.liquidacion.

drop function if exists public.fn_liquidar_sorteo(uuid, smallint);

create function public.fn_liquidar_sorteo(
  p_sorteo_id      uuid,
  p_numero_ganador smallint
) returns table (
...
create or replace function public.fn_es_servicio()
returns boolean
language sql
stable
as $$
...
create or replace function public.fn_exige(p_roles public.rol_usuario[])
returns void
language plpgsql
stable
security definer
...
revoke execute on function public.fn_auditar(text, uuid, text, text, text, text)
  from authenticated;
...
begin
  ...
  perform public.fn_auditar('sorteo', p_sorteo_id, 'abrir', 'estado', v_estado::text, 'abierto');
end;
$$;
...
begin
  ...
  foreach v_hora in array array['11:00', '15:00', '20:00']::public.hora_sorteo[]
  loop
    v_time := case v_hora
  ...
  perform public.fn_auditar('sorteo', null, 'programar_dia', 'fecha', null, p_fecha::text);
  return v_creados;
end;
$$;
...
--
--     revoke execute on function public.fn_auditar(...) from authenticated;
--
...
revoke execute on function public.fn_exige(public.rol_usuario[]) from authenticated;
revoke execute on function public.fn_es_servicio() from authenticated;
...
create or replace function public.fn_impacto_numero(
  p_sorteo_id uuid,
...
begin
  perform public.fn_exige(array['administrador', 'auditor']::public.rol_usuario[]);
...
create or replace function public.fn_peor_escenario(p_sorteo_id uuid)
returns table (
  numero        smallint,
  pago          numeric,
...
create or replace view public.v_agregado_sorteo_vendedor
...
comment on view public.v_agregado_sorteo_vendedor is
  'Todo indicador del sistema sale de aquí, y esto sale de las líneas. Ningún total se captura a mano.';

grant select on public.v_agregado_sorteo_vendedor to authenticated;

-- Índices que sostienen la agregación.
create index if not exists linea_ticket_numero on public.linea (ticket_id, numero);
create index if not exists ticket_sorteo_vigente on public.ticket (sorteo_id) where anulado_en is null;
...
create or replace function public.fn_resumen_mensual(
  p_desde date,
  p_hasta date
) returns table (
  anio            integer,
...
as $$
  ...
  left join public.v_agregado_sorteo_vendedor a
...
as $$
  ...
  left join public.v_agregado_sorteo_vendedor a on a.sorteo_id = s.id
...
create or replace function public.fn_reporte_totales(
  p_desde       date,
  p_hasta       date,
  p_vendedor_id uuid    default null,
  p_hora        public.hora_sorteo default null,
...
create or replace function public.fn_bitacora_vendedor(
  p_vendedor_id uuid,
  p_fecha       date,
  p_hora        public.hora_sorteo default null
) returns table (
...
create or replace function public.fn_actividad_horaria(
  p_vendedor_id uuid,
  p_fecha       date
) returns table (
  hora_reloj integer,
...
create or replace function public.fn_crear_lote_ocr(
  p_imagen_path      text,
  p_vendedor_id      uuid,
  p_sorteo_id        uuid,
...
declare
  v_id uuid;
begin
  perform public.fn_exige(array['digitador', 'administrador']::public.rol_usuario[]);

  insert into public.lote_ocr (
  ...
  perform public.fn_auditar('lote_ocr', v_id, 'extraer', 'costo_inferencia',
                           null, p_costo::text);
  return v_id;
end;
$$;
...
create or replace function public.fn_validar_lote_ocr(
  p_lote_id uuid,
  p_lineas  jsonb
...
as $$
declare
  v_lote    public.lote_ocr%rowtype;
  v_suma    numeric(14,2);
  v_ticket  record;
  v_n       integer;
begin
  ...
  select * into v_lote
  from public.lote_ocr where id = p_lote_id
  for update;
  ...
  from public.fn_registrar_ticket(
    ...
    p_lineas,
    null, null, null,
    'ocr'::public.canal_ticket,
    p_lote_id
  );

  select jsonb_array_length(p_lineas) into v_n;

  update public.lote_ocr
  set estado = 'validado', validado_por = auth.uid(), validado_en = now()
  where id = p_lote_id;

  perform public.fn_auditar('lote_ocr', p_lote_id, 'validar', 'folio',
...
create or replace function public.fn_rechazar_lote_ocr(
  p_lote_id uuid,
  p_motivo  text
) returns void
language plpgsql
...
begin
  ...
  perform public.fn_auditar('lote_ocr', p_lote_id, 'rechazar', 'motivo', null, p_motivo);
end;
$$;
...
create or replace function public.fn_gasto_ocr(
  p_desde date,
  p_hasta date
) returns table (
  lotes             integer,
...
create table if not exists public.limite_franja (
  hora           public.hora_sorteo primary key,
  limite_casa    numeric(14,2) not null,
...
comment on table public.limite_franja is
...
insert into public.limite_franja (hora, limite_casa) values
  ('11:00', 4000),
  ('15:00', 5000),
  ('20:00', 7000)
on conflict (hora) do nothing;

alter table public.limite_franja enable row level security;

create policy limite_franja_lectura on public.limite_franja
  for select to authenticated using (true);
...
create or replace function public.fn_ciclo_sorteos()
returns table (accion text, fecha date, hora public.hora_sorteo)
language plpgsql
security definer
set search_path = public
as $$
...
begin
  ...
  perform public.fn_programar_dia(v_hoy);
  perform public.fn_programar_dia(v_manana);
  ...
  loop
    ...
    from public.limite_franja l where l.hora = v_s.hora;

    perform public.fn_abrir_sorteo(v_s.id, coalesce(v_limite, 5000));
  ...
  for v_s in
    ...
    order by s.hora_cierre
  loop
    perform public.fn_cerrar_sorteo(v_s.id);
...
revoke execute on function public.fn_ciclo_sorteos() from public, anon, authenticated;
...
select cron.unschedule('ciclo-sorteos')
where exists (select 1 from cron.job where jobname = 'ciclo-sorteos');

select cron.schedule(
  'ciclo-sorteos',
  '*/5 * * * *',
  $cron$ select public.fn_ciclo_sorteos(); $cron$
);
...
create or replace function public.fn_reparar_cupo(p_sorteo_id uuid)
returns integer
language plpgsql
security definer
set search_path = public
...
begin
  ...
  from public.sorteo s where s.id = p_sorteo_id
  for update;
  ...
  select l.limite_casa into v_limite
  from public.limite_franja l where l.hora = v_hora;
  ...
  select p_sorteo_id,
         ...
         coalesce((
           select sum(li.monto)
           from public.linea li
           join public.ticket t on t.id = li.ticket_id
           where t.sorteo_id = p_sorteo_id
             and t.anulado_en is null
             and li.numero = n
         ), 0)
  ...
  if v_faltan > 0 then
    perform public.fn_auditar('sorteo', p_sorteo_id, 'reparar_cupo', 'numeros',
                             null, v_faltan::text);
  end if;
...
begin
  ...
  for v_s in
    ...
    where s.estado in ('abierto', 'cerrado')
      and (select count(*) from public.cupo_numero c where c.sorteo_id = s.id) < 100
    order by s.fecha, s.hora
  loop
    v_repar := public.fn_reparar_cupo(v_s.id);
    if v_repar > 0 then
...
create or replace function public.fn_sembrar_vendedores_demo(p_cuantos integer default 25)
returns integer
language plpgsql
security definer
set search_path = public
...
begin
  ...
  for i in 1..least(p_cuantos, array_length(v_nombres, 1)) loop
    ...
    insert into public.vendedor (codigo, nombre, ciudad, barrio, zona, color,
                                lat, lng, telefono)
    values (
...
create or replace function public.fn_sembrar_dia_demo(p_fecha date)
returns table (hora public.hora_sorteo, tickets integer, lineas integer, venta numeric)
language plpgsql
security definer
set search_path = public
as $$
...
begin
  ...
  foreach v_h in array array['11:00','15:00','20:00']::public.hora_sorteo[]
  loop
    ...
    from public.sorteo s where s.fecha = p_fecha and s.hora = v_h;

    -- Idempotente: un sorteo que ya tiene ventas no se vuelve a sembrar.
    if exists (select 1 from public.ticket t where t.sorteo_id = v_sorteo limit 1) then
      continue;
    end if;
    ...
    -- sorteo con ventas.
    update public.sorteo s set estado = 'abierto'
    ...
    from (select l.ticket_id, sum(l.monto) suma
          from public.linea l join _tk k on k.id = l.ticket_id
...
create or replace function public.fn_borrar_demo()
returns text
language plpgsql
security definer
set search_path = public
...
begin
  ...
  delete from public.liquidacion lq
  where lq.sorteo_id in (
    select s.id from public.sorteo s
    where s.fecha < v_hoy
      and not exists (select 1 from public.ticket t where t.sorteo_id = s.id));

  delete from public.cupo_numero c
  where c.sorteo_id in (
    select s.id from public.sorteo s
    where s.fecha < v_hoy
      and not exists (select 1 from public.ticket t where t.sorteo_id = s.id));

  delete from public.sorteo s
  where s.fecha < v_hoy
    and not exists (select 1 from public.ticket t where t.sorteo_id = s.id);
  get diagnostics v_s = row_count;

  delete from public.parametro_vendedor p
  using public.vendedor v
  where v.id = p.vendedor_id and v.codigo ~ '^V-1[0-9]{2}$';

  delete from public.vendedor v where v.codigo ~ '^V-1[0-9]{2}$';
...
revoke execute on function public.fn_sembrar_vendedores_demo(integer) from public, anon, authenticated;
revoke execute on function public.fn_sembrar_dia_demo(date) from public, anon, authenticated;
revoke execute on function public.fn_borrar_demo() from public, anon, authenticated;
...
--
-- La función declara `returns table (hora public.hora_sorteo, …)`, así que
-- `hora` es también una variable de salida. En
--
--     on conflict (fecha, hora) do nothing
--
-- PostgreSQL no puede decidir si `hora` es la columna de `public.sorteo` o la
...
begin
  ...
  loop
    ...
    select p_fecha, v_h,
           ...
           'abierto'
    where not exists (
      select 1 from public.sorteo s where s.fecha = p_fecha and s.hora = v_h);
...
create or replace function public.fn_azar(p_semilla text)
...
comment on function public.fn_azar(text) is
...
begin
  ...
  loop
    ...
    v_dec  := floor(public.fn_azar(v_sal || 'dec') * 10)::int * 10;
    v_hot1 := floor(public.fn_azar(v_sal || 'h1') * 100)::int;
    v_hot2 := floor(public.fn_azar(v_sal || 'h2') * 100)::int;

    -- Uno de cada diez sorteos "corre el dato" y un número se lleva una tajada
    -- enorme. Es el suceso que produce los meses en pérdida: sin él, con
    -- noventa sorteos al mes la ley de los grandes números aplana el resultado.
    if public.fn_azar(v_sal || 'carga') < 0.10 then
      v_cargado := floor(public.fn_azar(v_sal || 'cn') * 100)::int;
      v_pcarga  := 0.25 + public.fn_azar(v_sal || 'cp') * 0.20;
    else
      v_cargado := null;
      v_pcarga  := 0;
    end if;
    ...
    with nuevos as (
      ...
      select
        'D' || to_char(p_fecha, 'YYMMDD')
             ...
             || '-' || lpad(g::text, 3, '0'),
        v_sorteo, v.id, 'movil', 1,
        v_inicio + (public.fn_azar(v.id::text || v_sal || g::text || 'h') * v_dur),
        -- El vendedor se mueve por su zona; no vende siempre en el mismo metro.
        v.lat + (public.fn_azar(v.id::text || v_sal || g::text || 'la') - 0.5) * 0.012,
        v.lng + (public.fn_azar(v.id::text || v_sal || g::text || 'ln') - 0.5) * 0.012
      from public.vendedor v
      cross join lateral generate_series(
        1,
        greatest(1, round(8 * (0.4 + public.fn_azar(v.id::text || v_sal || 'nt') * 1.2))::int)
      ) g
      where v.activo
      returning id, vendedor_id
    )
    ...
    join public.parametro_vendedor p
      on p.vendedor_id = t.vendedor_id and p.vigente_hasta is null
    cross join lateral generate_series(
      1, 1 + floor(public.fn_azar(t.id::text || 'nl') * 8)::int) g
    cross join lateral (
      select public.fn_azar(t.id::text || g::text || 'a') r,
             public.fn_azar(t.id::text || g::text || 'b') r2,
             public.fn_azar(t.id::text || g::text || 'c') r3
    ) x;
...
revoke execute on function public.fn_azar(text) from public, anon;
revoke execute on function public.fn_sembrar_dia_demo(date) from public, anon, authenticated;
...
-- SÍNTOMA
-- -------
-- Los primeros días tardaban ~450 ms. Hacia el día 207, con `public.linea` ya en
...
--     from public.linea l join _tk k on k.id = l.ticket_id
--
-- sale más barato recorrer `public.linea` entera y hacer un hash join que
...
begin
  ...
  loop
    ...
    -- Con clave primaria: el join contra `public.linea` tiene por dónde entrar.
    ...
    -- recorrer `public.linea` entera antes que entrar por `linea_ticket`.
    analyze _tk;
...
-- son también variables, y chocan con las columnas homónimas de `public.linea`
...
create or replace function public.fn_utilidad_por_numero(p_sorteo_id uuid)
...
comment on function public.fn_utilidad_por_numero(uuid) is
...
create or replace function public.fn_desliquidar_demo(p_sorteo_id uuid)
returns text
language plpgsql
security definer
set search_path = public
...
begin
  ...
  delete from public.liquidacion lq where lq.sorteo_id = p_sorteo_id;
  ...
  perform public.fn_auditar('sorteo', p_sorteo_id, 'desliquidar_demo', 'estado',
                           'liquidado', 'cerrado');
...
revoke execute on function public.fn_utilidad_por_numero(uuid) from public, anon;
revoke execute on function public.fn_desliquidar_demo(uuid) from public, anon, authenticated;
...
begin
  ...
  delete from public.linea l
  where l.ticket_id in (
    select t.id from public.ticket t where t.folio ~ '^D[0-9]{6}');
  get diagnostics v_l = row_count;

  delete from public.liquidacion lq
  where lq.sorteo_id in (
    select s.id from public.sorteo s where s.fecha < v_hoy);

  delete from public.ticket t where t.folio ~ '^D[0-9]{6}';
  get diagnostics v_t = row_count;
...
create index if not exists ticket_creado_en
  on public.ticket (creado_en desc);
...
analyze public.linea;
analyze public.ticket;
analyze public.sorteo;
analyze public.cupo_numero;
...
-- mil filas por la red y un recorrido de `public.linea` por cada carga.
--
-- Contar es exactamente lo que la base hace mejor.

create or replace function public.fn_mapa_dia(
  p_fecha       date,
  p_vendedor_id uuid default null
)
returns table (
...
comment on function public.fn_mapa_dia(date, uuid) is
...
create or replace function public.fn_vendido_por_vendedor(p_sorteo_id uuid)
returns table (
  r_vendedor_id uuid,
  r_numero      smallint,
  r_vendido     numeric
...
comment on function public.fn_vendido_por_vendedor(uuid) is
...
revoke execute on function public.fn_mapa_dia(date, uuid) from public, anon;
revoke execute on function public.fn_vendido_por_vendedor(uuid) from public, anon;
...
--           select 1 from public.ticket t
--           where t.id = linea.ticket_id
--             and ( t.vendedor_id = public.fn_vendedor_actual()
--                   or public.fn_rol_actual() in (...) )))
--
...
drop policy if exists ticket_propio on public.ticket;

create policy ticket_propio on public.ticket
  for select to authenticated
  using (
    (select public.fn_rol_actual()) in ('administrador', 'auditor', 'digitador')
    or vendedor_id = (select public.fn_vendedor_actual())
  );

drop policy if exists linea_por_ticket on public.linea;
...
create policy linea_por_ticket on public.linea
  ...
  using (
    ...
    or exists (
      select 1 from public.ticket t
      where t.id = linea.ticket_id
        and t.vendedor_id = (select public.fn_vendedor_actual())
    )
  );
...
as $$
  ...
  conteos as (
    select
      ...
      (select count(*) from public.sorteo s
       where s.fecha between p_desde and p_hasta
         and s.estado = 'liquidado'
...
create unique index if not exists usuario_vendedor_unico
  on public.usuario (vendedor_id) where vendedor_id is not null;
...
begin
  ...
  insert into public.usuario (usuario, nombre, rol, vendedor_id, hash)
  ...
  perform public.fn_auditar('usuario', v_id, 'crear', 'rol', null, p_rol::text);
...
create or replace function public.fn_autenticar(p_usuario text, p_contrasena text)
returns table (
  r_id          uuid,
  r_nombre      text,
  r_rol         public.rol_usuario,
  r_vendedor_id uuid,
...
begin
  ...
  update public.usuario set ultimo_acceso = now() where id = v_id;

  return query
    select u.id, u.nombre, u.rol, u.vendedor_id, u.debe_cambiar
    from public.usuario u where u.id = v_id;
end;
$$;

-- --- Cambio de contraseña ---------------------------------------------------

create or replace function public.fn_cambiar_contrasena(
  p_usuario_id  uuid,
  p_actual      text,
  p_nueva       text
)
...
begin
  ...
  perform public.fn_auditar('usuario', p_usuario_id, 'cambiar_contrasena', 'hash',
                           null, null);
end;
$$;
...
begin
  ...
  perform public.fn_auditar('usuario', p_usuario_id, 'restablecer_contrasena', 'hash',
                           null, null);
end;
$$;
...
create or replace function public.fn_usuario(p_id uuid)
returns table (
  r_id          uuid,
  r_usuario     text,
  r_nombre      text,
...
as $$
  ...
  from public.usuario u where u.id = p_id;
$$;

/** Qué vendedores ya tienen acceso, para no ofrecer el alta dos veces. */
create or replace function public.fn_accesos_vendedor()
...
create or replace function public.fn_mi_dia(p_vendedor_id uuid, p_fecha date)
returns table (
  r_sorteo_id uuid,
  r_hora      public.hora_sorteo,
...
revoke execute on function public.fn_crear_usuario(text, text, text, public.rol_usuario, uuid)
  from public, anon, authenticated;
revoke execute on function public.fn_autenticar(text, text) from public, anon, authenticated;
revoke execute on function public.fn_cambiar_contrasena(uuid, text, text) from public, anon, authenticated;
revoke execute on function public.fn_restablecer_contrasena(uuid, text) from public, anon, authenticated;
revoke execute on function public.fn_usuario(uuid) from public, anon, authenticated;
revoke execute on function public.fn_accesos_vendedor() from public, anon, authenticated;
revoke execute on function public.fn_mi_dia(uuid, date) from public, anon, authenticated;
revoke execute on function public.fn_mis_tickets(uuid, date, integer) from public, anon, authenticated;
...
-- encontró y siguió.
--
-- Las funciones declaran `set search_path = public, extensions`, así que `crypt` y
...
create or replace function public.fn_control_actividad(
  p_desde      date,
  p_hasta      date,
  p_vendedores uuid[] default null
)
...
create or replace function public.fn_bitacora_rango(
  p_vendedor_id uuid,
  p_desde       date,
  p_hasta       date,
  p_hora        public.hora_sorteo default null,
...
revoke execute on function public.fn_control_serie(date, date, uuid[], public.hora_sorteo)
  from public, anon;
revoke execute on function public.fn_control_actividad(date, date, uuid[]) from public, anon;
revoke execute on function public.fn_bitacora_rango(uuid, date, date, public.hora_sorteo, integer)
  from public, anon;
...
--
--     from public.vendedor v
--     left join public.ticket t on t.vendedor_id = v.id and t.anulado_en is null
--     left join public.sorteo s on s.id = t.sorteo_id
--                             and s.fecha between p_desde and p_hasta
--     left join public.linea  l on l.ticket_id = t.id and s.id is not null
--
...
-- -------------------------------------
-- `public.liquidacion` no es un total capturado a mano: lo escribe
...
create index if not exists liquidacion_sorteo on public.liquidacion (sorteo_id);

analyze public.liquidacion;
...
--
-- `public.vendedor.activo` existe desde 0001 con un comentario explícito —
...
alter table public.vendedor add column if not exists eliminado_en timestamptz;

comment on column public.vendedor.eliminado_en is
...
create index if not exists vendedor_vigente
  on public.vendedor (codigo) where eliminado_en is null;

-- --- Bajas y altas ---------------------------------------------------------

create or replace function public.fn_desactivar_vendedor(
  p_vendedor_id uuid,
...
as $$
declare
  v_vendedor public.vendedor%rowtype;
begin
  perform public.fn_exige(array['administrador']::public.rol_usuario[]);
  ...
  update public.vendedor set activo = false where id = p_vendedor_id;
  update public.usuario   set activo = false where vendedor_id = p_vendedor_id;

  perform public.fn_auditar('vendedor', p_vendedor_id, 'inactivar', 'activo',
                           'true', 'false');
end;
$$;

create or replace function public.fn_activar_vendedor(
  p_vendedor_id uuid,
  p_usuario_id  uuid default null
) returns void
language plpgsql
...
begin
  ...
  update public.vendedor set activo = true where id = p_vendedor_id;
  update public.usuario   set activo = true where vendedor_id = p_vendedor_id;

  perform public.fn_auditar('vendedor', p_vendedor_id, 'reactivar', 'activo',
                           'false', 'true');
end;
$$;

create or replace function public.fn_eliminar_vendedor(
  p_vendedor_id uuid,
  p_usuario_id  uuid default null
) returns void
language plpgsql
...
begin
  ...
  perform public.fn_auditar('vendedor', p_vendedor_id, 'eliminar', 'eliminado_en',
                           null, now()::text);
end;
$$;

comment on function public.fn_eliminar_vendedor(uuid, uuid) is
...
create or replace function public.fn_sesion_vigente(p_usuario_id uuid)
returns boolean
language sql
stable
security definer
...
as $$
  select exists (
    select 1
    from public.usuario u
    left join public.vendedor v on v.id = u.vendedor_id
...
comment on function public.fn_sesion_vigente(uuid) is
...
revoke execute on function public.fn_desactivar_vendedor(uuid, uuid)
  from public, anon, authenticated;
revoke execute on function public.fn_activar_vendedor(uuid, uuid)
  from public, anon, authenticated;
revoke execute on function public.fn_eliminar_vendedor(uuid, uuid)
  from public, anon, authenticated;
revoke execute on function public.fn_sesion_vigente(uuid)
  from public, anon, authenticated;
revoke execute on function public.fn_restablecer_contrasena(uuid, text)
  from public, anon, authenticated;
...
-- public.liquidacion con venta, comisión, premios y utilidad. Lo que no existía
...
-- puede saltarse un sorteo suelto de en medio. public.liquidacion ya es única
...
create table if not exists public.corte_vendedor (
  id          uuid primary key default gen_random_uuid(),
  vendedor_id uuid not null references public.vendedor(id),
  desde       date not null,
  hasta       date not null,
...
comment on table public.corte_vendedor is
...
comment on column public.corte_vendedor.saldo is
...
create table if not exists public.corte_detalle (
  corte_id       uuid not null references public.corte_vendedor(id) on delete cascade,
  liquidacion_id uuid not null references public.liquidacion(id),
...
create index if not exists corte_vendedor_por_vendedor
  on public.corte_vendedor (vendedor_id, pagado_en desc);

alter table public.corte_vendedor enable row level security;
...
create or replace function public.fn_liquidacion_pendiente(
  p_vendedor_id uuid,
  p_desde       date,
  p_hasta       date
) returns table (
...
comment on function public.fn_liquidacion_pendiente(uuid, date, date) is
...
create or replace function public.fn_cortes_vendedor(
  p_vendedor_id uuid,
...
as $$
  ...
  from public.corte_vendedor c
  where c.vendedor_id = p_vendedor_id
  order by c.pagado_en desc
  limit greatest(p_limite, 1);
$$;

comment on function public.fn_cortes_vendedor(uuid, integer) is
...
revoke execute on function public.fn_liquidacion_pendiente(uuid, date, date)
  from public, anon, authenticated;
revoke execute on function public.fn_registrar_corte(uuid, uuid[], date, date, text, uuid)
  from public, anon, authenticated;
revoke execute on function public.fn_cortes_vendedor(uuid, integer)
  from public, anon, authenticated;
...
--    de public.liquidacion se quedaría con el total viejo. De ahí
...
alter table public.ticket add column if not exists forzado boolean not null default false;

comment on column public.ticket.forzado is
  'Registrado por administración con la venta ya cerrada. Se audita aparte y se distingue en pantalla.';

create index if not exists ticket_forzado on public.ticket (sorteo_id) where forzado;
...
declare
  ...
  v_premios  numeric(14,2);
begin
  select * into v_sorteo from public.sorteo where id = p_sorteo_id;
  ...
  perform public.fn_auditar('liquidacion', p_sorteo_id, 'recalcular', 'venta',
                           null, v_venta::text);
end;
$$;
...
comment on function public.fn_registrar_ticket(uuid, uuid, jsonb, double precision, double precision, uuid, public.canal_ticket, uuid, boolean, uuid) is
...
comment on function public.fn_registrar_tanda(uuid, uuid, jsonb, double precision, double precision, boolean, uuid) is
...
revoke execute on function public.fn_recalcular_liquidacion(uuid, uuid)
  from public, anon, authenticated;
revoke execute on function public.fn_registrar_tanda(uuid, uuid, jsonb, double precision, double precision, boolean, uuid)
  from public, anon;
...
revoke execute on function public.fn_mi_periodo(uuid, date, date)
  from public, anon, authenticated;
...
-- guarda `public.ticket.creado_en`, no la del reloj del dispositivo. Un
...
--
-- Es, término a término, lo que ya guarda public.liquidacion: venta, comision,
...
comment on function public.fn_informe_gerencia(date, date) is
  'El informe de gerencia, una fila por vendedor: venta, premiado, factor, pago, comisión, bruto y neto. Sólo sorteos liquidados.';

revoke execute on function public.fn_informe_gerencia(date, date)
  from public, anon;
...
drop function if exists public.fn_informe_gerencia(date, date);
...
-- ------------------------
-- Se lee de public.liquidacion, así que un sorteo sin número ganador todavía no
...
create or replace function public.fn_analisis_resultados(
  p_desde       date,
  p_hasta       date,
  p_grano       text,
  p_vendedor_id uuid default null,
...
comment on function public.fn_analisis_resultados(date, date, text, uuid, public.hora_sorteo) is
  'Resultado real agregado al grano pedido: dia, semana, mes o anio. Sólo sorteos liquidados.';

revoke execute on function public.fn_analisis_resultados(date, date, text, uuid, public.hora_sorteo)
  from public, anon;
...
drop function if exists public.fn_analisis_resultados(date, date, text, uuid, public.hora_sorteo);

create function public.fn_analisis_resultados(
  p_desde       date,
  p_hasta       date,
  p_grano       text,
  p_vendedor_id uuid default null,
...
-- --------------------------------------------------------------------------
create or replace function public.fn_semanas_operadas()
returns table (
  r_inicio   date,
  r_fin      date,
  r_semana   integer,
...
comment on function public.fn_semanas_operadas() is
  'Una fila por semana con movimiento liquidado, de la más reciente a la más vieja.';

revoke execute on function public.fn_semanas_operadas() from public, anon;
...
-- --------------------------------------------------------------------------
create or replace function public.fn_historial_vendedor(
  p_vendedor_id uuid
) returns table (
  r_inicio   date,
  r_fin      date,
...
comment on function public.fn_historial_vendedor(uuid) is
  'La historia de un vendedor semana a semana. El acumulado es la suma de estas filas.';

revoke execute on function public.fn_historial_vendedor(uuid) from public, anon;
...
create or replace function public.fn_resultado_por_dia_semana()
returns table (
...
comment on function public.fn_resultado_por_dia_semana() is
  'Todos los lunes juntos, todos los martes juntos: una fila por día de la semana, toda la historia.';

revoke execute on function public.fn_resultado_por_dia_semana() from public, anon;
...
create or replace function public.fn_liquidacion_por_semana(
  p_vendedor_id uuid default null
) returns table (
  r_inicio        date,
  r_fin           date,
...
revoke execute on function public.fn_liquidacion_por_semana(uuid) from public, anon;
...
-- >>>>>>>>>>>>>>>>>>>>  migrations/0045_saldo_arrastrado.sql  <<<<<<<<<<<<<<<<<<<<
...
-- >>>>>>>>>>>>>>>>>>>>  migrations/0046_semana_completa_con_abonos.sql  <<<<<<<<<<<<<<<<<<<<
...
-- --------------------------------------------------------------------------
create or replace function public.fn_abonos_semana(
  p_vendedor_id uuid,
  p_desde       date,
  p_hasta       date
) returns table (
...
revoke execute on function public.fn_abonos_semana(uuid, date, date) from public, anon;
...
create table if not exists public.venta_total (
  id                 uuid primary key default gen_random_uuid(),
  sorteo_id          uuid not null references public.sorteo(id),
  vendedor_id        uuid not null references public.vendedor(id),
  venta              numeric(14,2) not null,
...
comment on table public.venta_total is
  'Venta capturada por totales, sin detalle de números. Para cuando el vendedor no usó el portal.';
comment on column public.venta_total.premios is
  'El premio total de esa venta. No se verifica contra el número ganador: no hay números que verificar.';

create index if not exists venta_total_sorteo on public.venta_total (sorteo_id)
  where anulado_en is null;
create index if not exists venta_total_vendedor on public.venta_total (vendedor_id, sorteo_id)
  where anulado_en is null;
...
-- liquide. En los dos casos, todo lo que lee de `public.liquidacion` —que es
...
comment on function public.fn_registrar_venta_total(uuid, uuid, numeric, numeric, text, uuid) is
...
begin
  ...
  perform public.fn_auditar('venta_total', p_id, 'anular', null, null, null, p_usuario_id);
...
revoke execute on function public.fn_anular_venta_total(uuid, uuid) from public, anon;
...
--
-- Aquí se cambian las DOS funciones que escriben en `public.liquidacion`, que
...
--
-- La 0047 llamaba a `public.fn_auditar(...)` pasándole el usuario como séptimo
...
-- primer registro con «function public.fn_auditar(...) does not exist».
--
...
-- >>>>>>>>>>>>>>>>>>>>  migrations/0050_una_captura_viva_de_verdad.sql  <<<<<<<<<<<<<<<<<<<<
...
create unique index if not exists venta_total_una_viva
  on public.venta_total (sorteo_id, vendedor_id)
  where anulado_en is null;

comment on index public.venta_total_una_viva is
  'Una sola captura por totales viva por vendedor y sorteo. Parcial: las anuladas no cuentan.';

-- >>>>>>>>>>>>>>>>>>>>  migrations/0051_saldos_por_vendedor.sql  <<<<<<<<<<<<<<<<<<<<
...
-- >>>>>>>>>>>>>>>>>>>>  migrations/0053_contacto_opcional.sql  <<<<<<<<<<<<<<<<<<<<
...
--
-- Sin esto la instalación queda completa pero inaccesible: no hay forma de
-- entrar y, por tanto, tampoco de crear el primer usuario desde la interfaz.
--
-- LA CONTRASEÑA SE GUARDA CIFRADA, nunca en claro. `crypt` con `gen_salt('bf')`
-- es bcrypt con sal propia por fila, que es lo mismo que usa `fn_crear_usuario`
-- para el resto de las cuentas: así ésta no es un caso especial que se
-- comporte distinto al validarse.
--
-- `on conflict do nothing` para que volver a ejecutar el instalador no falle
-- ni pise una contraseña ya cambiada.
--
-- ┌───────────────────────────────────────────────────────────────────────┐
-- │  usuario: admin        contraseña: admin123                           │
-- │                                                                       │
-- │  ES UNA CLAVE DE ARRANQUE, y de las primeras que prueba cualquiera.   │
-- │  Queda como permanente por decisión explícita, así que el sistema NO  │
-- │  va a pedir cambiarla: mientras siga puesta, cualquiera que llegue a  │
-- │  la dirección entra como administrador y ve —y cobra— toda la         │
-- │  operación. Conviene cambiarla desde «Cambiar contraseña» en cuanto   │
-- │  el sistema deje de ser una prueba.                                   │
-- └───────────────────────────────────────────────────────────────────────┘
-- ===========================================================================

insert into public.usuario (usuario, nombre, rol, hash, debe_cambiar)
values (
  'admin',
  'Administrador',
  'administrador',
...
do $$
begin
  if exists (select 1 from public.usuario where usuario = 'admin') then
    raise notice 'Listo. Entre con admin / admin123 y cambie la contraseña.';
  else
    raise exception 'No se pudo crear el usuario administrador.';
  end if;
end $$;
