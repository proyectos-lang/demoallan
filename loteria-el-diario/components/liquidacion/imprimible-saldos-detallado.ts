/**
 * LA HOJA DETALLADA — CONSERVADA, HOY SIN USAR.
 *
 * Es el imprimible que había antes de la hoja simplificada: seis columnas con
 * saldo de la semana y liquidado, más la nota que explica cada una.
 *
 * Se guarda entero y en un archivo aparte —no comentado ni en un `.bak`—
 * porque el usuario pidió poder volver a él. Comentado se pudre en silencio;
 * así al menos compila y `npx tsc` avisa si algo de lo que usa cambia.
 *
 * PARA VOLVER A ÉL: en `tabla-saldos.tsx`, importar `documentoSaldosDetallado`
 * de aquí en vez de `documentoSaldos` de `imprimible-saldos.ts`. Nada más; la
 * forma de `HojaSaldos` es la misma en los dos.
 */

import type { HojaSaldos } from "@/components/liquidacion/imprimible-saldos";
 ...
 */
export function documentoSaldosDetallado(h: HojaSaldos): string {
